...
exports.PollyEnvironmentFactory = PollyEnvironmentFactory;
...
function PollyEnvironmentFactory(JestEnvironment, PollyConstructor = _core.Polly) {
  return class PollyEnvironment extends JestEnvironment {
    async setup() {
      await super.setup();
      this.pollyGlobals = new _common.JestPollyGlobals(this.global);
    ...
    async handleTestEvent(event, state) {
      ...
      if (event.name === 'test_start' && this.pollyGlobals.isPollyActive) {
        const recordingName = getRecordingName(event.test, state.rootDescribeBlock);
        this.pollyGlobals.pollyContext.polly = new PollyConstructor(recordingName, this.pollyGlobals.pollyContext.options);
